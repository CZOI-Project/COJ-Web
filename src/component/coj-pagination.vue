...
<template>
    <div>
        <div class="mdui-btn-group">
            ...
            <button v-for="i in range(l,r)" type="button" class="mdui-btn" :class="{'mdui-btn-active': i==props.now}" @click="props.callback(3,i)">
                {{ i }}
            </button>
...
<script setup lang="ts" name="coj-pagination">
    ...
    function update(){
        ...
        //边界触碰情况
        if(l.value < 1) l.value = 1,r.value = maxn.value;
        if(r.value > props.total) r.value = props.total,l.value = props.total - maxn.value + 1;
    }
