        ...
        <v-col cols="12" sm="8">
          ...
          <v-row dense>
            <v-col cols="12" v-for="(announcement, index) in announcements" v-bind:key="announcement.title">
              <v-card @click="showAnnouncement(index)" class="mx-auto mdui-ripple" :subtitle="announcement.time" :title="announcement.title" :color="announcement.top? 'teal':''"
                variant="tonal">
                ...
                <template v-slot:append>
                  <v-avatar size="24">
                    <v-img :src="COJConfig.resourceURL + '/avatar/' + announcement.user + '.png'"></v-img>
                  </v-avatar>
                </template>
              </v-card>
            </v-col>
...
onMounted(async () => {
  const response_announcements = await axios.get(COJConfig.resourceURL + "/announcements/index.json");
  Object.assign(announcements, response_announcements.data);
  const response_countdown = await axios.get(COJConfig.resourceURL + "/countdown.json");
  Object.assign(countdown, response_countdown.data);
  for(let i = 0; i < countdown.length; i ++){
    let d = parseDateTime(countdown[i].time);
    //console.log(countdown[i].time);
    countdown[i].time = d.getMonth() + "." + d.getDay();
...
async function showAnnouncement(target:number) {
  ...
  const res = await axios.get(COJConfig.resourceURL + "/announcements/" + announcements[target].target);
  now.value = 1;
