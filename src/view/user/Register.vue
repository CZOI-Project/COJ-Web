<template>
  <div style="padding-top:50px">
    <div class="coj-main mdui-shadow-1">
      <v-row dense>
        <v-col cols="12" sm="6" style="display: flex; align-items: center;" :class="{'coj-suitable1': screenWidth >= 600 && step == 1}">
          <div class="coj-title">
            创建账号
              ...
              <v-sheet class="mdui-m-t-5">
                <v-form ref="form">
                  ...
                  <div class="coj-clearfix coj-main-right-btns">
                    <button @click="router.push('/user/login')" class="mdui-btn mdui-ripple" :disabled="loading">已有账号
                    </button>
                    <v-btn
                        color="teal"
...
import {userEmailService, userRegisterService} from '@/api/user';
...
async function sendCode() {
  ...
  loading.value = true;
  try {
    await userEmailService(registerData.email);
    step.value = 2;
  } catch (err) {
    console.log(err)
  }
