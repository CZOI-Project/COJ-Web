        ...
        <v-row dense>
          ...
          <v-col col="12" sm="6">
            ...
            <div class="coj-clearfix coj-main-right-btns">
              <v-btn color="teal" append-icon="mdi-arrow-right" variant="flat" border="0" :loading="loading"
                @click="verifyAction()">
                下一步
              </v-btn>
            </div>
      ...
      <div class="coj-main mdui-shadow-1" style="padding: 20px" v-else-if="step == 2">
        ...
        <div style="margin-top: 20px;margin-left: 30px;margin-right: 30px;">
          ...
          <ul class="mdui-list mdui-list-dense">
            <li v-for="item in list" class="mdui-list-item mdui-ripple" @click="choose(item.id)">
        ...
        <v-row dense>
          ...
          <v-col cols="12" sm="6">
            <v-sheet style="margin-top: 70px">
              <v-form ref="form">
                <v-text-field v-model="name" :rules="validName" label="真实姓名" required :disabled="loading"></v-text-field>
                <v-text-field v-model="code" :rules="validCode" class="mdui-m-t-1" label="邀请码" required
                  :disabled="loading"></v-text-field>
                <div class="coj-clearfix coj-main-right-btns">
                  <button @click="step = 1" class="mdui-btn mdui-ripple" :disabled="loading">上一步
                  </button>
                  <v-btn color="teal" append-icon="mdi-arrow-right" variant="flat" border="0" :loading="loading"
                    class="mdui-m-l-1" @click="normalVerify()">
                    验证
                  </v-btn>
                </div>
              </v-form>
...
import type { User, VerifyEntry } from "@/entry";
...
let step = ref(1);
let type = ref(1);
let list = reactive([] as VerifyEntry[]);
let target_id = ref('');
let code = ref('');
let name = ref('');
...
async function verifyAction() {
  if (type.value == 1) {
    loading.value = true;
    try {
      list = Object.assign(list, await request.get("/user/getVerifyList"));
    } catch (err) {
      console.log(err)
    }
    loading.value = false;
...
async function choose(target: number) {
  target_id.value = target.toString();
  code.value = await request.get("/user/quickVerify", toGetParams({ id: target_id.value, option: 0 }))
  step.value = 4;
}
async function quickVerify() {
  loading.value = true;
  try {
    await request.get("/user/quickVerify", toGetParams({ id: target_id.value, option: 1 }))
    //到这里相当于验证成功了，再次获取个人信息
    userStore.entry = await request.get("/user/detail");
    mdui.snackbar({
...
async function normalVerify() {
  ...
  loading.value = true;
  try {
    await request.get("/user/verify", toGetParams({ code: code.value, name: name.value }))
    userStore.entry = await request.get("/user/detail");
    mdui.snackbar({
      message: userStore.entry.name + "，您好！欢迎回来！"
    });
